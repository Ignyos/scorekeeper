(function () {
  ...
  async function renderThreeToThirteenPage(db, deps) {
    ...
    if (existingViewportController && typeof existingViewportController.abort === "function") {
      existingViewportController.abort();
    }
    ...
    function teardownScoreInputFocusManager() {
      ...
      if (typeof window.commitScoreValue === "function" && window.commitScoreValue.__scorekeeperOwner === "ttt") {
    ...
    function setupScoreInputFocusManager() {
      ...
      commitScoreValue.__scorekeeperOwner = "ttt";
    ...
    async function showEndResults() {
      ...
      const winningTotal = totals[sortedPlayerIds[0]];
      const winningPlayerNames = sortedPlayerIds
        .filter((playerId) => totals[playerId] === winningTotal)
