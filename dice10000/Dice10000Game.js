const DICE_10000_DEFAULT_STATE = {
  rounds: [],
...
class Dice10000Game {
  ...
  validateActiveRoundScoreUpdate(update) {
    ...
    if (update.value !== null && (!Number.isInteger(update.value) || update.value < 0)) {
  ...
  validateCompletedRoundScoreUpdate(update) {
    ...
    if (!Number.isInteger(update.value) || update.value < 0) {
      throw new Error("Score must be a non-negative integer");
    }
  }
...
window.ScorekeeperGames.Dice10000Game = Dice10000Game;
