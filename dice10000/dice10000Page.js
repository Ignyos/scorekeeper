(function () {
  ...
  async function renderDice10000Page(db, deps) {
    const {
      parseSessionId,
      listPlayers,
      renderShell,
    ...
    if (!sessionId) {
      ...
      renderShell("", startGameModalHtml("Dice 10,000", players));
      ...
      submitButton?.addEventListener("click", async () => {
        try {
          ...
          const GameClass = await loadGameClassBySlug("dice10000");
          const game = new GameClass(null);
          game.ensurePlayers(selected);

          const session = await createSession(db, {
            game: "dice10000",
            gameClass: "Dice10000Game",
            gameVersion: "1",
            playerIds: selected,
            gameState: game.getState(),
          });

          window.location.href = withSessionId(routePath("dice10000"), session.id);
        } catch (error) {
          alert(error.message);
        }
      });
    ...
    const rulesAction = rulesTriggerHtml("dice10000", { context: "game", sessionId: session.id });
    ...
    renderShell(
      ...
      `
        ...
        <section class="card">
          <p class="muted">Enter each player's banked score per round. First to 10,000 triggers the final round.</p>
          <div class="yahtzee-sheet-wrap">
            <table class="scrabble-sheet">
              ...
              <tbody id="dice10000-scoreboard-body"></tbody>
            </table>
          </div>
          <div class="row game-actions-row">
            ${rulesAction}
            <button type="button" id="dice10000-end-game" ${session.status !== "active" ? "disabled" : ""}>End Game</button>
          </div>
        </section>

        <div class="modal-backdrop" id="dice10000-end-confirm-modal" hidden>
          <div class="modal" role="dialog" aria-modal="true" aria-labelledby="dice10000-end-confirm-title">
            <h2 id="dice10000-end-confirm-title">End Dice 10,000 Game</h2>
            <p id="dice10000-end-confirm-text">Are you sure you want to end the game?</p>
            ...
            <div class="row start-game-actions scrabble-end-confirm-actions">
              <button type="button" id="dice10000-end-confirm-home">Home</button>
              <button type="button" id="dice10000-end-confirm-submit">End Game</button>
              <button type="button" id="dice10000-end-confirm-cancel">Cancel</button>
            </div>
          </div>
        </div>

        <div class="modal-backdrop" id="dice10000-end-results-modal" hidden>
          <div class="modal" role="dialog" aria-modal="true" aria-labelledby="dice10000-end-results-title">
            <h2 id="dice10000-end-results-title">Final Results</h2>
            <ul id="dice10000-end-results-list" class="end-game-results"></ul>
            <div class="row start-game-actions">
              <button type="button" id="dice10000-end-results-close">Close</button>
            </div>
          </div>
        </div>
      `,
    ...
    const scoreboardBody = document.getElementById("dice10000-scoreboard-body");
    const endGameButton = document.getElementById("dice10000-end-game");

    const endConfirmModal = document.getElementById("dice10000-end-confirm-modal");
    const endConfirmHome = document.getElementById("dice10000-end-confirm-home");
    const endConfirmSubmit = document.getElementById("dice10000-end-confirm-submit");
    const endConfirmCancel = document.getElementById("dice10000-end-confirm-cancel");

    const endResultsModal = document.getElementById("dice10000-end-results-modal");
    const endResultsList = document.getElementById("dice10000-end-results-list");
    const endResultsClose = document.getElementById("dice10000-end-results-close");
    ...
    async function commitCompletedCellEdit(roundIndex, playerId, rawValue) {
      ...
      if (!Number.isInteger(parsed) || parsed < 0) {
        alert("Enter a valid non-negative integer score.");
        return false;
      }
    ...
    function renderScoreboardRows() {
      ...
      const totalCells = session.playerIds
        .map((playerId) => {
          ...
          const reachedGoal = total >= 10000;
          return `<td><strong>${total}${reachedGoal ? "*" : ""}</strong></td>`;
        })
        .join("");
  ...
  window.ScorekeeperGamesUI.renderDice10000Page = renderDice10000Page;
})();
